...
const SingleBook = () => {
  ...
  const getBook = async () => {
    try {
      const res = await axiosInstance.get(`/book/get/${id}`);
      console.log(res);
      setBook(res.data);
    } catch (e) {}
  };
    ...
    <div className="w-full flex gap-8">
      <div className="w-1/4">
        <img
          src={
            book.cover ||
            'https://dw0i2gv3d32l1.cloudfront.net/uploads/stage/stage_image/40050/optimized_large_thumb_stage.jpg'
          }
          alt=""
        ...
        <div className="w-full bg-background3 h-fit rounded-xl mt-8 p-3 flex gap-2 align-baseline">
          <h1 className="text-2xl font-semibold text-secondary">
            45th{' '}
            <span className="font-normal text-lg text-text1">
              dbinder ranking
            </span>
          </h1>
        </div>
      ...
      <div className="w-3/4">
        <h1 className="text-4xl text-white font-bold ">{book.title}</h1>
